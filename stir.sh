...
function init_env() {
  ...
  PROJNAME PROJCLIENT DEVURL PRODURL REPO MASTER STAGING PRODUCTION COMMITMSG \
  DEPLOY DONOTDEPLOY TASK CHECKBRANCH ACTIVECHECK CHECKTIME GARBAGE WFCHECK \
  ACFKEY ACF_LOCK WFOFF REMOTELOG REMOTETEMPLATE LOCALHOSTPOST LOCALHOSTPATH \
  DIGESTEMAIL DIGESTSLACK DIGESTURL CLIENTLOGO REMOTEURL SCPPOST SCPUSER \
  SCPHOST SCPHOSTPATH SCPHOSTPORT SCPPASS SCPCMD SSHCMD LOGMSG EXPIRELOGS \
  SERVERCHECK STASH MAILPATH REQUIREAPPROVAL ADDTIME SCPPORT TASKUSER \
