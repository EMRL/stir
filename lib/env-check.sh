...
function update_global() {
  ...
  env_settings=(CLEAR_SCREEN WORK_PATH CONFIG_DIR CHECK_SERVER CHECK_ACTIVE \
    ...
    GRAVITY_FORMS_LICENSE ACF_KEY ACTIVATE_PLUGINS NIKTO NIKTO_CONFIG \
    NIKTO_PROXY)
...
function update_project() { 
  ...
  env_settings=(PROJECT_NAME PROJECT_CLIENT DEV_URL PROD_URL REPO_HOST REPO MASTER \
    ...
    MONITOR_URL MONITOR_USER MONITOR_PASS SERVER_ID STAGING_DEPLOY_PATH \
    PRODUCTION_DEPLOY_HOST PRODUCTION_DEPLOY_PATH SCP_DEPLOY_USER \
    SCP_DEPLOY_PASS SCP_DEPLOY_PORT DB_BACKUP_PATH PREPARE PREPARE_CONFIG \
    ACF_LOCK STAGING GRAVITY_FORMS_LICENSE INCLUDE_DETAILS MAUTIC_URL \
    MAUTIC_AUTH MAUTIC_LEGACY_VERSION SKIP_GIT NEWS_URL ACF_COMPOSER \
    WP_CLI_PATH COMPOSER_CLI_PATH ACTIVATE_PLUGINS)
