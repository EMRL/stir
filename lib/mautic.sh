...
function test_mautic() { 
  if [[ -n "${MAUTIC_AUTH}" ]]; then
    ...
    mautic_payload="$(${curl_cmd} --silent --get "${MAUTIC_URL}api/emails?limit=1&orderBy=id&orderByDir=desc" --header "Authorization: Basic ${MAUTIC_AUTH}")"; error_check
    ...
    mtc_type="$(echo ${mautic_payload} | get_json_value emailType 1)"
