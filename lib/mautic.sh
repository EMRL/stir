...
function mtc_data_loop() {
  ...
  for i in {1..3} ; do
    for j in "${mtc_var[@]}" ; do
      ...
      working_readRate="$(get_percent ${!working_sentCount:-} ${!working_readCount:-})"
...
function mtc_test() {
  trace "${curl_cmd} --silent --get \"${MAUTIC_URL}api/emails?limit=3&orderBy=id&orderByDir=desc\" --header \"Authorization: Basic ${MAUTIC_AUTH}\""
  mtc_data_loop
