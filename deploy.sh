...
read -r CLEARSCREEN WORKPATH CONFIGDIR REPOHOST WPCLI SMARTCOMMIT GITSTATS \
  ...
  WFOFF REMOTELOG LOCALHOSTPOST LOCALHOSTPATH DIGESTEMAIL DIGESTSLACK DIGESTURL \
  CLIENTLOGO REMOTEURL SCPPOST SCPUSER SCPHOST SCPHOSTPATH SCPPASS SCPCMD SSHCMD \
  LOGMSG EXPIRELOGS SERVERCHECK STASH MAILPATH REQUIREAPPROVAL ADDTIME TASKUSER \
  CLIENTID CLIENTSECRET REDIRECTURI AUTHORIZATIONCODE ACCESSTOKEN REFRESHTOKEN \
  PROFILEID METRIC RESULT ALLOWROOT SHORTEMAIL DIGESTCOVER INCOGNITO REPORTURL \
  CLIENTCONTACT INCLUDEHOSTING <<< ""
